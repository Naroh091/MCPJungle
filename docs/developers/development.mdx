...
- **Development UI**: embedded browser UI assets and build workflow in `ui/`
...
The built-in development UI lives in `ui/`.

Its static assets are committed to `ui/dist/` and embedded into the Go server binary.

If you change files under `ui/src/`, rebuild the embedded assets with:
...
Then run the usual Go tests or local server startup flow.
...
If you want to work on the browser UI too:
