...
When MCPJungle runs in development mode, the built-in web UI is also served at `/` and uses this same management API.
...
## Development UI endpoint

When the server is running in **development mode**, MCPJungle also serves a built-in web UI at:

```text
GET /
```

This UI is not available in enterprise mode in v1.
